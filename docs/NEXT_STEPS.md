...
6. **属性测试 + 自动缩减**：在生成器之外随机长事件序列跑 FakeLLM，失败时缩到最短复现序列（§13 第 4 条剩下的一半）。
7. **解析 warning 回喂**：`<invoke>` 别名目前只进 trace 不回喂；评估是否值得作为 rejected 行让模型自我纠正。

## 产品线

8. 流式输出（解析器按整段工作，需要增量解析或先流后解析）。
9. 记忆检索式召回（条目多时全量注入会撑 context）。
10. 并发写同一会话的锁（多进程 CLI 同时进同一 `--session`）。
