...
- ~~`extractJsonObject` 从 `protocol/parser.ts` 导出，`consolidate.ts` 不再拷贝。~~ → Python 版已做：`mini_agent/protocol/parser.py::extract_json_object` 唯一实现，`consolidate.py` 直接 import。
...
6. ~~属性测试 + 自动缩减~~ → **模型层**已做（#5 B2：`mini_agent/machine/explore.py` 随机游走 + ddmin，只用表 + interpret）。剩下**运行层**：随机长事件序列真跑 FakeLLM，失败时缩到最短 FakeLLM 脚本；绿 = 一条故意注入的 runtime bug 被缩到 ≤3 步复现。
