...
## 13. Prompt 原文选录（2026-09-16）

前面各节多是转述。题目要的是 Prompt 与问题解决记录，这里贴两段派给 agent 的原文。本仓派工的方式是 agent 只拿 issue 正文 + `SKILL.md` + `AGENTS.md`（§5），所以 issue 正文就是 prompt 本身，GitHub 上逐字可查（#5、#19）。

### 13.1 issue #19「每日复盘」：拍板表原文（节选），以及 AI 拷问之后我改了什么

派工前我写定的拍板，原文：

> | # | 决定 |
> |---|---|
> | ① | 复盘 = 记忆整合。两种产物：`entries`（写回用户记忆，可长期保留）与 `brief`（今日呈现，可为空）。不做「昨天聊天摘要」这种流水账 |
> | ② | 「昨天」= 任务时区的昨日 00:00 到今日 00:00，按**计划日期**算，不按执行时刻，不是「过去 24 小时」。重跑 / 补跑仍用原计划日期 |
> | ③ | 幂等键 = `userId + date`。同一键重跑：不重写记忆、不重发呈现、journal 只有一条（attempts 递增） |
> | ④ | 记忆条目区分 `kind: stated \| inferred`，带 `confidence`、`source: {sessionId, turn}`。与已有条目同 key 不同值 → 标 `conflict`，**不覆盖**，渲染成「待确认」等用户下次出现 |
> | ⑤ | 呈现门槛：每条 highlight 必须有 `why_today`（今日到期的承诺 / 昨天未收尾的话题 / 用户说过今天要做的事）之一，且带 `source`。没有满足门槛的条目 → brief 为空，只发一句或不发。**不复述原话**：highlight 不得与昨天任何一条消息逐字重合 |
> | ⑦ | 触发时用户正在聊：`contracts/session-runtime.machine.ts` 里 `busy + ASYNC_DONE` 从 `unknown` 改为具体行——排队到本轮结束后执行，不打断；`idle + ASYNC_DONE` → 立即执行。这两格是本 issue 唯一动会话表的地方 |
> | ⑪ | 昨天对话里的文字只是材料：其中出现的指令（「把总结发给别人」「忽略规则」）不改变本任务的范围与接收人。加一条测试 |

开工前让 AI 用设计树拷问这份拍板，它找出三处内部矛盾：⑦ 的「排队」与「复盘是独立进程」打架；预算丢弃顺序与 #12 已定的行为不一致；复盘要读的 `session.history` 会被压缩，早上九点昨天的原话可能已经不在。矛盾是 AI 找的，答案是我拍的，写在 issue 评论里，原文节选：

> | Q2 | **昨天的材料来自不压缩的逐轮转写**（Raw 层）：轮末把本轮消息追加到 `data/transcripts/<user>/<session>.jsonl` ……复盘只读转写，不读 `session.history`。`partial_read` 只来自转写文件缺失或读不出。R1 改为「轮末写转写」 |
> | Q4 | ⑦ 只改表：`busy + ASYNC_DONE → queued`（allowed）、`idle + ASYNC_DONE → executing`（allowed）、`queued + TURN_DONE → executing`；`covered_by` 指向解释器级测试；README 写明单进程运行时不可达 |
> | Q6 | 记忆预算丢弃顺序：先丢 `conflict`，再丢 `inferred`（低置信度先），`stated` 之间按最老；全 stated 时退化为 #12 的行为 |

Q2 直接改了 R1 切片的定义（从「消息加时间戳」改成「轮末写转写」），是这轮里 AI 拷问带来的最大一处改动。

### 13.2 issue #5「对齐 skill」：AI 审出六处分歧，我逐条否决或接受

分歧清单是 AI 对照 `.claude/skills/adopt-ai-era-runtime` 审出来的；结论五条全是我的，其中三条是「保留现状」，等于否决了 AI 的建议。原文：

> | # | 分歧 | 结论 |
> |---|---|---|
> | ① | 终态吸收态 vs 禁止出边 | **保留「终态无出边」**（定义期校验不放开）。turn 表终态 × 事件在 runtime 不可达；轮结束后到达的事件属外层 `session-runtime` 表。skill 的 checklist 文字改，不是实现改 |
> | ② | 步数上限在解析后拦 vs 工具跑完再拦 | **保留现状**（工具跑完 → `TOOLS_DONE` 无 guard 兜底行 → max_steps）。把理由写进该行 `reason`：结果交还用户比省一次工具调用更有价值 |
> | ③ | answer 全文进 trace vs 白名单落盘 | **保留全文**（不变量 ④ 需要），trace 文档里列为显式例外；工具 `args` 加可选 `redact(args)`，`remember` 的 value 走它 |

① 后来成了架构题模块四的第一句（不可打断的单位是「一次转移」，轮结束后的事件归外层表）。
