...
from ..llm.types import NATIVE, TEXT
...
def build_system_prompt(tools: List[dict], memory_block: str, mode: str = TEXT) -> str:
    memory = "\n关于当前用户的长期记忆（跨会话）：\n" + memory_block if memory_block else ""
    if mode == NATIVE:
        return _build_native_system_prompt(memory)
    tool_list = "\n".join(f"- {t['name']}: {t['description']}\n  参数 Schema: {json.dumps(t['parameters'], ensure_ascii=False, separators=(',', ':'))}" for t in tools)
    return "\n".join([
...
def _build_native_system_prompt(memory: str) -> str:
    return "\n".join([
