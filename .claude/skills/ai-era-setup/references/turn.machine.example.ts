...
export const turnMachine = defineMachine<TurnFacts>({
  ...
  transitions: [
    ...
    { id: "t-tools", from: "deciding", event: "PARSED_TOOL_CALLS", to: "executing_tools", kind: "allowed", p0: true, effects: { tools: true }, invariants: ["no-tool-after-parse-error"], note: "不设步数 guard：最后一步的工具照跑，结果交还用户比省一次调用更有价值（2026-09-15 拍板 ②）", covered_by: ["test/unit/agent-loop.test.ts::调用工具：结果以 tool 消息回填后模型再给最终答案"] },
    ...
    { id: "t-tools-done", from: "executing_tools", event: "TOOLS_DONE", guard: "hasStepsLeft", to: "deciding", kind: "allowed", p0: true, covered_by: ["test/unit/agent-loop.test.ts::调用工具：结果以 tool 消息回填后模型再给最终答案"] },
    { id: "t-tools-done-cap", from: "executing_tools", event: "TOOLS_DONE", to: "max_steps", kind: "allowed", p0: true, invariants: ["terminal-exclusive"], note: "无 guard 兜底：工具结果已回填但步数用尽，以「已达上限 + 最近三条工具结果」结束", covered_by: ["test/unit/agent-loop.test.ts::模型一直调工具时，到达单轮最大步数就停下并把已有信息交还用户"] },
  ],
  invariants: {
