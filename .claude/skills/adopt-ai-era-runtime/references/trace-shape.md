...
- 白名单落盘：密钥不进；模型输出只进 `preview`（截断）；工具 `args` 经工具自己的 `redact(args)`（如 remember 的 value 只留长度）。
- **显式例外：终态转移的 `answer` 副作用存全文**——不变量「返回值 == 盘上历史末条 == trace 末次决策」需要逐字对齐，截断就对不上；answer 是产品输出不是用户输入（2026-09-15 拍板 ③）。
